...
public interface TaskRepository extends JpaRepository<Task, Long> {
    ...
    List<Task> findAllByGroupAndAssignee(Group group, com.calendarapp.model.User user);
    
    @Query("SELECT t FROM Task t WHERE t.group = :group AND t.assignee = :user AND t.done = true")
    List<Task> findAllByGroupAndAssigneeAndDoneTrue(Group group, com.calendarapp.model.User user);
    
    @Query("SELECT t FROM Task t WHERE t.group = :group AND t.assignee = :user AND t.done = false")
    List<Task> findAllByGroupAndAssigneeAndDoneFalse(Group group, com.calendarapp.model.User user);
}